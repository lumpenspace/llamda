...
from test.function_fixtures import get_weather, get_weather_with_param_annotations
...
def test_llamda_decorator():
    ...
    assert schema["description"] == "Retrieve the weather information for a given location and date."
    assert schema["parameters"]["properties"]["location"]["description"] == "The location for which to retrieve the weather information."
    assert schema["parameters"]["properties"]["date"]["description"] == "The date for which to retrieve the weather information."
    assert schema["parameters"]["required"] == ["location"]  # "date" is optional, so it shouldn't be in the "required" list

def test_llamda_decorator_with_docstring_params():

    # Test the decorated function
    result = get_weather_with_param_annotations(location="Philadelphia", date="2023-06-01")
    assert result.success
...
def test_llamda_decorator_missing_description():
    with pytest.raises(ValueError, match="Description missing for function 'get_weather_missing_description'"):
        @llamda()
...
def test_llamda_decorator_missing_param_description():
    with pytest.raises(ValueError, match="Description missing for function 'get_weather_missing_param_description'"):
        @llamda(date="The date for which to retrieve the weather information.")
        def get_weather_missing_param_description(location: str, date: Optional[str]) -> str:
            return get_weather(location, date)

def test_llamda_decorator_missing_type_annotation():
    with pytest.raises(ValueError, match="Type annotation missing for parameter 'location' in function 'get_weather_missing_type_annotation'"):
        @llamda(main="Retrieve the weather information for a given location and date.", location="The location for which to retrieve the weather information.", date="The date for which to retrieve the weather information.")
